...
def calc_breed(dino, weight, food):
    ...
    # CRUMPLECORN EXACT FORMULA (Line 2611-2613, 2839-2846, 2658-2665)
    total_mat = d["base_mat"] / m["mat"]
    juv_time = total_mat * 0.1
    
    # Calculate food rates (Line 2611-2613)
    ...
    # getfoodforperiod() - Line 2839-2846 - NO WEIGHT MULTIPLICATION!
    def get_food_for_period(start, end):
        end = min(total_mat, end)
        end = max(start, end)
        start_food_rate = max_food_rate - food_rate_decay * start
        end_food_rate = max_food_rate - food_rate_decay * end
        total_time = end - start
        return 0.5 * total_time * (start_food_rate - end_food_rate) + end_food_rate * total_time
    
    # Baby phase food (0 to 10%) - Line 2659
    baby_food_points = get_food_for_period(0, juv_time)
    baby_food_items = baby_food_points / fv  # Line 2663 - divide by (food_value * foodmult)
    
    # TOTAL food to adult (0 to 100%) - Line 2658
    total_food_points = get_food_for_period(0, total_mat)
    total_food_items = total_food_points / fv  # Line 2662
...
async def on_ready():
    ...
    print(f"🎯 Crumplecorn-accurate (NO weight multiplication)")
...
if __name__ == "__main__":
    ...
    if not token:
        try:
            ...
            print("❌ No token!")
            exit(1)
    print("🚀 Starting ARK Breeding Bot...")
    client.run(token)
