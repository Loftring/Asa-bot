...
# CRUMPLECORN GLOBAL CONSTANT (Line 2405)
...
def calc_breed(dino, weight, food):
    ...
    # CRUMPLECORN EXACT FORMULA (Line 2492-2497, 2839-2846)
    total_mat = d["base_mat"] / m["mat"]
    juv_time = total_mat * 0.1
    
    # Calculate food rates (Line 2492-2494)
    max_food_rate = d["food_drain"] * m["consumption"]
    min_food_rate = BASE_MIN_FOOD_RATE * (d["food_drain"] / (d["food_drain"] / BASE_MIN_FOOD_RATE))
    food_rate_decay = (max_food_rate - min_food_rate) / total_mat
    
    # Baby phase food (0 to 10%) - Trapezoid formula
    start_food_rate_baby = max_food_rate
    end_food_rate_baby = max_food_rate - food_rate_decay * juv_time
    baby_food_points = 0.5 * juv_time * (start_food_rate_baby - end_food_rate_baby) + end_food_rate_baby * juv_time
    baby_food_items = (baby_food_points * weight) / fv
    
    # TOTAL food to adult (0 to 100%)
    start_food_rate_total = max_food_rate
    end_food_rate_total = max_food_rate - food_rate_decay * total_mat
    total_food_points = 0.5 * total_mat * (start_food_rate_total - end_food_rate_total) + end_food_rate_total * total_mat
    total_food_items = (total_food_points * weight) / fv
...
class TimerView(View):
    def __init__(self, stats, timer_id):
        super().__init__(timeout=None)
        self.stats = stats
        self.timer_id = timer_id
        self.add_item(Button(label="⏹️ Stop Timer", style=discord.ButtonStyle.danger, custom_id=f"stop_{timer_id}"))
    ...
    async def callback(self, i):
        ...
        async def start_timer_callback(btn_i):
            ...
            timer_embed = discord.Embed(
                ...
                description=f"**Time to Adult:** {fmt_time(s['adult_time'])}\n**Ends:** <t:{int((datetime.utcnow() + timedelta(seconds=s['adult_time'])).timestamp())}:R>",
...
@tree.command(name="timers", description="⏱️ View active breeding timers")
