...
def calc_breed(dino, weight, food):
    ...
    # CRUMPLECORN EXACT - Line 2611-2613
    total_mat = d["base_mat"] / m["mat"]
    juv_time = total_mat * 0.1
    
    # CORRECT food rate calculation!
    ...
    # getfoodforperiod - Line 2839-2846
...
async def dinos(i: discord.Interaction):
    ...
    if carns:
        e.add_field(name=f"🥩 Carnivores ({len(carns)})", value=", ".join(carns[:30]), inline=False)
    if herbs:
        e.add_field(name=f"🥕 Herbivores ({len(herbs)})", value=", ".join(herbs[:30]), inline=False)
    if omnis:
...
async def timers(i: discord.Interaction):
    ...
    for timer_id, timer in user_timers.items():
        ...
        e.add_field(name=f"🦖 {timer['dino']}", value=f"Progress: {progress:.1f}%\nRemaining: {fmt_time(remaining)}", inline=True)
